...
def getTraceroute(field1, value1, field2, value2, dt):
    q = {
      "size" : 10000,
      "sort": [
        {
    ...
    data = scan(client=hp.es, index='ps_trace', query=q)
    res = []
...
def getMeasures(dt, field1, value1, idx, field2=None, value2=None):
    q = {
          "size" : 10000,
          "sort": [
            {
...
#     print(str(q).replace("\'", "\""))
    ...
    data = scan(client=hp.es, index=idx, query=q, 
                _source=["timestamp","src","dest","src_host","dest_host","src_site","dest_site", "ipv6", "packet_loss"])
    res = []
    for item in data:
        res.append(item['_source'])
    ...
    if len(res) > 0:
        pl = pd.DataFrame(res)
        pl['dt'] = pd.to_datetime(pl['timestamp'], unit='ms')
